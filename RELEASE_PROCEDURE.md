...
- [ ] Update author-list in `docs/1.1-DRAFT/_metadata.liquid` based on team list
...
- [ ] Update `docs/1.1-DRAFT/ro-crate-metadata.json` (eat our own dogfood)
  - [ ] add new team members to both `author` on the root dataset, and `member` on the RO-Crate community entity
  - [ ] add any new terms used / update URIs if changed
  - [ ] update `isBasedOn`
- [ ] Update schema.org version in scripts/schema_version.py (make note of any important schema.org updates in the changelog)
...
- [ ] Update variables at the top of `Makefile` to reflect the release being prepared
- [ ] Run `make release` – this does the following steps:
  ...
  - [ ] Convert `docs/1.1/*.md` to single-page HTML and PDF using `pandoc`
  - [ ] Copy `examples` folder and find & replace 1.1-DRAFT -> 1.1, update version in folder name
